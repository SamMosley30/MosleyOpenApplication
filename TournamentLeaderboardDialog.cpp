...
TournamentLeaderboardDialog::TournamentLeaderboardDialog(const QString &connectionName, QWidget *parent)
    ...
    // Removed combinedOverallWidget
    , mosleyOpenWidget(new TournamentLeaderboardWidget(m_connectionName, this))      
    , twistedCreekWidget(new TournamentLeaderboardWidget(m_connectionName, this))  
    , day1LeaderboardWidget(new DailyLeaderboardWidget(m_connectionName, 1, this)) 
    , day2LeaderboardWidget(new DailyLeaderboardWidget(m_connectionName, 2, this)) 
    , day3LeaderboardWidget(new DailyLeaderboardWidget(m_connectionName, 3, this)) 
    , teamLeaderboardWidget(new TeamLeaderboardWidget(m_connectionName, this))     
    , cutLineLabel(new QLabel(tr("Cut Line Score (2-Day Mosley Net Stableford):"), this))
    , cutLineSpinBox(new QSpinBox(this))
    , applyCutButton(new QPushButton(tr("Apply Cut"), this))
    , clearCutButton(new QPushButton(tr("Clear Cut"), this))
    , refreshButton(new QPushButton(tr("Refresh All"), this)) 
    , closeButton(new QPushButton(tr("Close"), this))
    , exportImageButton(new QPushButton(tr("Export Current Tab"), this)) 
    ...
    , m_isCutApplied(false)                 
{
    ...
    // --- Setup Tab Widget ---
    // No "Combined Overall" tab anymore
    tabWidget->addTab(mosleyOpenWidget, tr("Mosley Open")); // Renamed for clarity
    tabWidget->addTab(twistedCreekWidget, tr("Twisted Creek")); // Renamed for clarity
...
void TournamentLeaderboardDialog::saveCutSettings() {
    ...
    if (!query.exec()) qWarning() << "TournamentLeaderboardDialog::saveCutSettings: Failed to save cut_line_score:" << query.lastError().text();

    query.bindValue(":key", SETTING_IS_CUT_APPLIED);
    query.bindValue(":value", m_isCutApplied);
    if (!query.exec()) qWarning() << "TournamentLeaderboardDialog::saveCutSettings: Failed to save is_cut_applied:" << query.lastError().text();
}
...
void TournamentLeaderboardDialog::cutLineScoreChanged(int value) {
    // Not currently used for auto-save, but could be.
}

void TournamentLeaderboardDialog::refreshLeaderboards() {
    qDebug() << "TournamentLeaderboardDialog: Refreshing. CutScore:" << m_cutLineScore << "IsCutApplied:" << m_isCutApplied;
