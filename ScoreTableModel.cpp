...
QVariant ScoreTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        // Columns 1-18 are for Hole Scores
        else if (index.column() >= 1 && index.column() <= 18) {
            ...
            if (m_scores.contains(player->id) && m_scores[player->id].contains(holeNum)) {
                ...
                if (score > 0) { // Assuming score 0 or less means not entered or invalid
        ...
        // Add logic for Total Score, Score vs Par columns later
        // else if (index.column() == 19) { /* Calculate and return Total Score */ }
        // else if (index.column() == 20) { /* Calculate and return Score vs Par */ }
    }

    // Handle other roles (e.g., TextAlignmentRole for score columns)
    if (role == Qt::TextAlignmentRole) {
        if (index.column() >= 1 && index.column() <= 18) { // Score columns
            return Qt::AlignCenter; // Center align scores
        }
        // Add alignment for other columns if needed
    }
    ...
    if (role == Qt::DisplayRole)
    {
        if (orientation == Qt::Horizontal) // Column headers
        {
            ...
            if (section == 0) {
                ...
                // You could also include Par here if you loaded hole details
                // Example: "Hole 1\n(Par 3)"
            ...
            else if (section == 19) { return "Total"; }
            // else if (section == 20) { return "Vs Par"; }

        }
        else // Vertical headers (row headers - usually not needed for this view)
        {
            // Section is the row index (0 to rowCount() - 1)
            // Returning empty QVariant means no vertical header is displayed by default
            // If you wanted row numbers or player IDs as vertical headers, you'd return them here.
            // return section + 1; // Example: return row number
        }
    }
...
bool ScoreTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    // You might add more validation here (e.g., max score)
    if (!ok || newScore <= 0) { // Assuming scores must be positive integers
        qDebug() << "ScoreTableModel::setData: Invalid score value entered:" << value.toString();
        // Optionally revert the view's change if validation fails
        // emit dataChanged(index, index, {role}); // Revert the view
    ...
    if (oldScore != newScore) {
        ...
        if (saveSuccess) {
            ...
            // Optional: Emit signals if total score or score vs par columns need updating
            // emit dataChanged(index.sibling(index.row(), getColumnForTotalScore()), index.sibling(index.row(), getColumnForTotalScore()), {Qt::DisplayRole});
            // emit dataChanged(index.sibling(index.row(), getColumnForVsPar()), index.sibling(index.row(), getColumnForVsPar()), {Qt::DisplayRole});

            qDebug() << "ScoreTableModel::setData: Score saved successfully for Player" << player->id << "Hole" << holeNum << "Score" << newScore;
        ...
        // Data didn't actually change, no need to save or emit signal
...
void ScoreTableModel::setCourseId(int courseId)
{
    ...
    if (m_currentCourseId == courseId || courseId <= 0) {
         ...
         if (m_currentCourseId > 0) { // Clear if we had a valid course selected before
...
void ScoreTableModel::loadActivePlayers()
{
    ...
    if (query.exec("SELECT id, name, handicap FROM players WHERE active = 1 ORDER BY name")) {
        ...
        qDebug() << "ScoreTableModel::loadActivePlayers: Loaded" << m_activePlayers.size() << "active players.";
    } else {
...
void ScoreTableModel::loadHoleDetails(int courseId)
{
    ...
    if (query.exec()) {
        ...
        qDebug() << "ScoreTableModel::loadHoleDetails: Loaded details for" << m_holeDetails.size() << "holes for course ID" << courseId;
    } else {
...
void ScoreTableModel::loadScores(int courseId)
{
    ...
    qDebug() << "ScoreTableModel::loadScores: Executing query:" << queryString; // Debug the final query string
    ...
    if (query.exec(queryString)) {
        ...
        qDebug() << "ScoreTableModel::loadScores: Loaded scores for day" << m_dayNum << "course" << courseId << ". Total score entries:" << m_scores.size();
    } else {
...
// === Optional: Implement submitAll/revertAll if using manual submit ===
/*
bool ScoreTableModel::submitAll()
{
    // This method would iterate through internal m_scores, find changes,
    // and save them to the database. More complex as you need to track
    // which scores have been modified since the last load/submit.
    qDebug() << "ScoreTableModel::submitAll() called (not implemented)";
    return false; // Not implemented
}

void ScoreTableModel::revertAll()
{
    // This method would discard pending changes in m_scores
    // by reloading scores from the database or reverting to cached original values.
    qDebug() << "ScoreTableModel::revertAll() called (not implemented)";
    // Not implemented
}
*/
